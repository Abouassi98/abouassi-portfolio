...
export const ar: SiteContent = {
  ...
  projects: [
    ...
    {
      id: 'ledger-service',
      name: 'ledger-service — دفتر قيد مزدوج',
      role: 'المؤلف · مشروع شخصي',
      period: '2026',
      domain: 'مفتوح المصدر · Java',
      summary:
        'خدمة صغيرة ومكتملة بملامح أنظمة الدفع: دفتر قيد مزدوج، وتحويلات آمنة عند التكرار، وسجل لا يقبل إلا الإضافة. كُتبت كعيّنة عمل، لأن عمل Spring المهني يقع داخل مستودعات عملاء خاصة.',
      points: [
        'القيد يتوازن لكل عملة على حدة، ويُتحقَّق منه في الباني — فلا يمكن إنشاء قيد غير متوازن أصلًا، لا مجرد منعه من الحفظ.',
        'المبالغ بـBigDecimal بدقة مثبّتة على العملة مع رفض التقريب، فأي كسر ضائع من القرش يرفع استثناءً بدل أن يختفي.',
        'تحويلات آمنة عند التكرار: مفتاح مع بصمة للطلب، فالإعادة المطابقة ترجع القيد الأصلي، والطلب المختلف يُرفض لا يُعاد تشغيله.',
        'أقفال صفوف تُؤخذ بترتيب المعرّفات، والرصيد يُقرأ بعد القفل. عشرون خيطًا تنطلق معًا تثبت غياب الصرف الزائد والتجمّد المتبادل.',
        'السجل لا يقبل إلا الإضافة على مستوى قاعدة البيانات؛ والتصحيح قيد معاكس، فيظهر الخطأ وتصحيحه معًا.',
        '69 اختبارًا، واختبارات التكامل على PostgreSQL حقيقية عبر Testcontainers لا على بديل في الذاكرة.',
      ],
    ...
    },
    {
      ...
      role: 'المؤلف · مشروع شخصي',
      period: '2026',
      domain: 'مفتوح المصدر · React وTypeScript',
      summary:
        'الموقع الذي تقرأه، مكتوب كعيّنة عمل بالتقنيات التي يدّعيها. محتواه مقيَّد بسجل حقائق، وقاعدتان من قواعد ذلك السجل يفرضهما نظام الأنواع ومجموعة الاختبارات بدل المراجعة اليدوية.',
      points: [
        'اتحاد مُميَّز يجعل أي مقياس ذي نطاق مُستعار يفشل في الترجمة ما لم يُسمِّ جهة العمل صاحبة ذلك النطاق.',
        'اختبار يُفشل البناء إذا عاد ادعاء محذوف أو مهارة غير مدعومة بدليل في أي من اللغتين، أو إذا تباعدت اللغتان.',
        'عربي وإنجليزي مع مُبدِّل اتجاه محفوظ يُطبَّق قبل أول رسم، وخصائص منطقية في كل التخطيط، وعزل ثنائي الاتجاه للأرقام.',
        'التبويبات ولوحات الطي تتبع أنماط WAI-ARIA؛ ومفاتيح الأسهم تتبع الترتيب البصري، فتعمل معكوسة في العربية عن قصد.',
        'Lighthouse 100 في كل المحاور بعد استبدال عائلتَي خطوط تحجبان الرسم بخط متغيّر واحد مُستضاف ذاتيًا.',
      ],
